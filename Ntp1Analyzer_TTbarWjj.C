#include "Ntp1Analyzer_TTbarWjj.h"
...
Ntp1Analyzer_TTbarWjj::Ntp1Analyzer_TTbarWjj( const std::string& dataset, const std::string& flags, TTree* tree ) :
     Ntp1Analyzer( "TTbarWjj", dataset, flags, tree ) {
  ...
  qglc_ = new QGLikelihoodCalculator("/afs/cern.ch/work/p/pandolf/public/ReducedHisto_2012.root");

} //constructor



void Ntp1Analyzer_TTbarWjj::CreateOutputFile() {
...
//reducedTree_->Branch("HLT_DoubleMu3", &HLT_DoubleMu3_, "HLT_DoubleMu3_/O");
  ...
  reducedTree_->Branch("passed_HLT_IsoMu24_eta2p1",   &passed_HLT_IsoMu24_eta2p1_,  "passed_HLT_IsoMu24_eta2p1_/O)");
  ...
  reducedTree_->Branch("matchedToHLTLept1",  &matchedToHLTLept1_,  "matchedToHLTLept1_/O");
  ...
  reducedTree_->Branch("qglJet", qglJet_, "qglJet_[nJets_]/F");
...
Ntp1Analyzer_TTbarWjj::~Ntp1Analyzer_TTbarWjj() {
...
void Ntp1Analyzer_TTbarWjj::Loop()
{
...
*/
     ...
     chargeLept1_ = muons[0].charge;
     combinedIsoRelLept1_ = muons[0].isolation;
     matchedToHLTLept1_ = isMatchedToHLT(muons[0].Eta(),muons[0].Phi(),0.3);
...
*/
     ...
     for( unsigned int iJet=0; iJet<nAK5PFPUcorrJet; ++iJet ) {
       ...
       thisJet.QGLikelihood2012 =  qglc_->computeQGLikelihood2012( thisJet.Pt(), thisJet.Eta(), rhoPF_, thisJet.nPFCand_QC_ptCut, thisJet.ptD_QC, thisJet.axis2_QC );
       ...
       if( thisJet.DeltaR(muons[0])<0.5 ) continue;
     ...
     for( unsigned iJet=0; iJet<leadJets.size() && nJets_<50; ++iJet ) {
       ...
       qglJet_[nJets_] = leadJets[iJet].QGLikelihood2012;
