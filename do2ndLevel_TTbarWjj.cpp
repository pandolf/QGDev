...
int main( int argc, char* argv[]) {
  ...
  if( argc!=2 && argc!=3 && argc!=4 ) {
    std::cout << "USAGE: ./do2ndLevel_TTbarWjj [dataset] [inputfile=\"\"] [flags=\"\"]" << std::endl;
    exit(31);
  }

  std::string dataset(argv[1]);

  Ntp1Analyzer_TTbarWjj* na;

  if( argc<4 ) {
    na = new Ntp1Analyzer_TTbarWjj(dataset);
  } else {
    std::string flags(argv[3]);
    na = new Ntp1Analyzer_TTbarWjj(dataset, flags);
  }
  ...
  if( isData2011 || isData2012 ) {
    ...
    if( dataset_tstr.BeginsWith("DoubleMu") ) {
      ...
      na->AddRequiredTrigger( "HLT_IsoMu24_eta2p1_v" );
